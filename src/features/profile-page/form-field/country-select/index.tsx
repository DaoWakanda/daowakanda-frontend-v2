...
export default function CountrySelect({ value, onChange, options }: CountrySelectProps) {
  ...
  const filteredCountries =
    query === ''
      ? options
      : options.filter((c) => c.label.toLowerCase().includes(query.toLowerCase()));
        ...
        <div className="relative">
          <div className="relative w-full cursor-default overflow-hidden rounded-lg bg-[#24252D] border border-[#49454F] text-left sm:text-sm">
            <Combobox.Input
              ...
              displayValue={() => selectedCountry?.label ?? ''}
          ...
          <Combobox.Options className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-[#24252D] py-1 text-white shadow-lg ring-1 ring-black ring-opacity-5">
            {filteredCountries.length === 0 && (
              <div className="cursor-default select-none px-4 py-2">No country found.</div>
            )}
