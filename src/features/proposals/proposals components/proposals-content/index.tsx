...
const ProposalsContent = () => {
  ...
  const fetchAllProposals = async (page: number) => {
    try {
  ...
  const handleNext = () => {
    if (pagination && currentPage < pagination.pageCount) setCurrentPage((prev) => prev + 1);
  };

  const goToPage = (page: number) => {
    if (pagination && page >= 1 && page <= pagination.pageCount) setCurrentPage(page);
  };
  ...
  const handleDelete = () => {
    if (selectedProposal) {
      // deleteProposal(selectedProposal); // Uncomment when using useProposals hook
      setShowDeleteModal(false);
    }
  };

  const handleClaimFunding = () => setShowSuccessModal(false);

  const safeProposals = Array.isArray(proposals) ? proposals : [];

  const filteredProposals = safeProposals
    .filter((proposal) => {
      if (activeTab === 'all') return true;
      if (activeTab === 'in-progress') return proposal.ongoing;
      if (activeTab === 'approved')
        return !proposal.ongoing && proposal.yesVotes.length > proposal.noVotes.length;
      if (activeTab === 'denied')
        return !proposal.ongoing && proposal.noVotes.length >= proposal.yesVotes.length;
    })
    .filter(
      (proposal) =>
  ...
  // Pagination items with optional dots
              ...
              <div className="flex items-center gap-1">
                ...
                {getPaginationItems().map((page, index) => (
                  <Button
                    ...
                    size="sm"
                    className={
                      page === currentPage ? 'bg-gray-700' : 'text-gray-400 border-gray-700'
                    }
                ...
                <Button
                  ...
                  className="text-gray-400 border-gray-700"
                  onClick={handleNext}
                  disabled={pagination ? currentPage === pagination.pageCount : true}
                >
                  Next
                </Button>
              </div>
