  ...
  return (
    <div
      className={classNames(
        ...
        'transition-all duration-300 relative p-1.5 gap-2 flex-wrap',
      )}
      style={{
  ...
  return (
    <div
      className={classNames(
        ...
        'transition-all duration-300 relative gap-4 flex-wrap',
      )}
    >
      <div
        className={classNames(
      ...
      {options.map((option, index) => {
        return (
          <button
            ...
            onClick={() => onSelect(typeof option === 'string' ? option : option.value)}
            className={classNames(
              'min-w-[76px] justify-center items-center py-[10.5px] px-[23px] relative z-2 transition-all duration-300',
              'font-degularDisplay text-[14px] font-medium whitespace-nowrap',
              indexOfSelectedOption === index ? 'text-[#FFF]' : 'text-[#171D26]',
            )}
            id={`${randomId}-${typeof option === 'string' ? option : option.value}-${index}`}
          >
            {typeof option === 'string' ? option : option.label}
